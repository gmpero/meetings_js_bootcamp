...
const objectDifference = (obj1, obj2) => {
    ...
    for (const key of keys) {
        if (obj1[key] !== obj2[key])
            resultObject[key] = obj1[key] ? obj1[key] : obj2[key];
    }
    return resultObject;
};
console.log(objectDifference({a: 1, b: 2, c: 3}, {a: 1, b: 4, d: 5}))
...
// → {c: 3, d: 4, a: 1, b: 2}
const rotateKeys = (object, positions) => {
...
console.log(rotateKeys({a: 1, b: 2, c: 3, d: 4}, 3));
...
const groupByRange = (object, arrayDiapasons) => {
    const resultObject = new Object();
    for (let i = 0; i < arrayDiapasons.length - 1; i++){
        resultObject[`${arrayDiapasons[i]}-${arrayDiapasons[i + 1]}`] = new Object;
        for (const [key, value] of Object.entries(object)) {
            if (value > arrayDiapasons[i] && value < arrayDiapasons[i + 1]) {
                resultObject[`${arrayDiapasons[i]}-${arrayDiapasons[i + 1]}`][key] = value;
            }
        }
    }
    return resultObject;
};
console.log(groupByRange({a: 15, b: 25, c: 35, d: 45}, [0, 20, 40, 60]));
